...
class SimpleRiskPredictor:
    ...
    def preprocess_data(self) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        clinical_relevance = {
            'Beta-blocker': {
                'vitals': {'heart_rate': 2.0, 'blood_pressure_systolic': 2.0},
                'labs': {'potassium': 1.0, 'creatinine': 1.0}
            },
            'ACE Inhibitor': {
                'vitals': {'blood_pressure_systolic': 2.0},
                'labs': {'potassium': 2.0, 'creatinine': 2.0}
            },
            'Anticoagulant': {
                'labs': {'platelets': 2.0, 'hemoglobin': 1.5}
            },
            'Antiarrhythmic': {
                'vitals': {'heart_rate': 2.0},
                'labs': {'qtc': 2.0, 'potassium': 1.5}
            },
            'Diuretic': {
                'labs': {'potassium': 2.0, 'creatinine': 2.0},
                'vitals': {'blood_pressure_systolic': 1.5}
            },
            'Antipsychotic': {
                'labs': {'qtc': 2.0, 'blood glucose': 1.5, 'liver enzymes': 1.5}
            },
            'Statin': {
                'labs': {'liver enzymes': 2.0, 'creatinine': 1.0}
            }
        }
    ...
    def train(self, epochs: int = 100, learning_rate: float = 0.01,
        ...
        optimizer = torch.optim.Adam(self.model.parameters(), 
                               ...
                               weight_decay=0.035)  # Increased from 0.01
        pos_weight = torch.tensor([0.8])  # Reduce weight of positive predictions
    ...
    def predict(self, patient_id: str, medication_id: str = None) -> Dict[str, float]:
        ...
        if medication_id:
            ...
            med_risk_factor += (interaction_count * 0.2 + contraindication_count * 0.3)
    ...
    def _get_feature_importance(self, X: torch.Tensor) -> Dict[str, float]:
        try:
            ...
            for name, weight, value in zip(feature_names, weights, X[0]):
                ...
                importance_dict[name] = float(importance)  # Ensure we return Python float
