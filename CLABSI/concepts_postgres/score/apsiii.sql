...
WITH pa AS (
  ...
  SELECT
    ...
    bg.charttime,
    bg.aado2,
    ROW_NUMBER() OVER (PARTITION BY ie.stay_id ORDER BY bg.aado2 DESC NULLS LAST) AS rn
  ...
  INNER JOIN mimiciv_derived.ventilation AS vd
    ...
    AND vd.ventilation_status = 'InvasiveVent'
  WHERE
    NOT vd.stay_id IS NULL /* patient is ventilated */
  ...
  SELECT
    ...
    END AS glucose_min, /* , labs.bicarbonate_min */ /* , labs.bicarbonate_max */
    vent.vent,
    ...
    gcs.gcs_verbal,
    gcs.gcs_eyes,
    gcs.gcs_unable, /* acute renal failure */
    arf.arf AS arf
  FROM mimiciv_icu.icustays AS ie
  ...
  SELECT
    ...
    CASE
      ...
      WHEN smax.creatinine_score < smin.creatinine_score
      THEN smin.creatinine_score
    END AS creatinine_score, /* the rule for BUN is the furthest from 0.. equivalent to the max value */
...
SELECT
  ...
  ie.hadm_id,
  ie.stay_id,
  apsiii, /* Calculate probability of hospital mortality using */ /* equation from Johnson 2014. */
