...
WITH blood_diff AS (
  SELECT
    ...
    le.specimen_id, /* create one set of columns for percentages, one set for counts */ /* we harmonize all count units into K/uL == 10^9/L */ /* counts have an "_abs" suffix, percentages do not */ /* absolute counts */
    MAX(CASE WHEN itemid IN (51300, 51301, 51755) THEN valuenum ELSE NULL END) AS wbc,
    MAX(CASE WHEN itemid = 52069 THEN valuenum ELSE NULL END) AS basophils_abs, /* 52073 in K/uL, 51199 in #/uL */
    MAX(
      CASE
        WHEN itemid = 52073
        THEN valuenum
        WHEN itemid = 51199
        THEN CAST(valuenum AS DOUBLE PRECISION) / 1000.0
        ELSE NULL
      END
    ) AS eosinophils_abs, /* 51133 in K/uL, 52769 in #/uL */
    MAX(
      CASE
        WHEN itemid = 51133
        THEN valuenum
        ...
        ELSE NULL
      END
    ) AS lymphocytes_abs, /* 52074 in K/uL, 51253 in #/uL */
    MAX(
      CASE
        WHEN itemid = 52074
        THEN valuenum
    ...
    MAX(CASE WHEN itemid = 52075 THEN valuenum ELSE NULL END) AS neutrophils_abs, /* convert from #/uL to K/uL */
    MAX(
      CASE
        WHEN itemid = 51218
        THEN CAST(valuenum AS DOUBLE PRECISION) / 1000.0
    ...
    MAX(CASE WHEN itemid = 51256 THEN valuenum ELSE NULL END) AS neutrophils, /* other cell count percentages */
