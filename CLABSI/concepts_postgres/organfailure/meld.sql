...
WITH cohort AS (
  ...
  SELECT
    ...
    bilirubin_total_max,
    inr_max,
    sodium_min, /* TODO: Corrected Sodium */
    CASE
    ...
    END AS sodium_score, /* if hemodialysis, value for Creatinine is automatically set to 4.0 */
    CASE
...
SELECT
  subject_id,
  hadm_id,
  stay_id, /* MELD Score without sodium change */
  meld_initial, /* MELD Score (2016) = MELD*10 + 1.32*(137-Na) – [0.033*MELD*10*(137-Na)] */
  CASE
    WHEN meld_initial > 11
    THEN meld_initial + 1.32 * sodium_score - 0.033 * meld_initial * sodium_score
    ELSE meld_initial
  END AS meld, /* original variables */
  rrt,
  creatinine_max,
  bilirubin_total_max,
  inr_max,
