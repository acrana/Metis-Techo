...
class RiskPredictor:
    def __init__(self):
        ...
        # Increased risk multipliers
        self.risk_multipliers = {
            'ade_base': 0.15,
            'ade_max': 0.45,
            'interaction_base': 0.2,
            'interaction_max': 0.5
        }
    ...
    def get_med_specific_features(self, cursor, patient_id: str, med_id: int) -> Dict:
        ...
        if interactions['interaction_count'] > 0:
            major_score = (interactions['major_count'] or 0) * 0.4  # Increased from 0.25
            moderate_score = (interactions['moderate_count'] or 0) * 0.25  # Increased from 0.15
            
            interaction_score = min(major_score + moderate_score, 0.75)  # Increased from 0.5
            
            total_meds = interactions['interaction_count']
            if total_meds > 3:
                interaction_score *= (1 + min(total_meds - 3, 3) * 0.15)  # Increased from 0.1
    ...
    def predict(self, cursor, patient_id: str, new_med_id: int = None) -> Dict[str, float]:
        ...
        if new_med_id:
            ...
            base_predictions = self.model(**features)
            
            # Enhanced risk calculations
            ade_risk = max(
                base_predictions[0, 0].item(),
                self.risk_multipliers['ade_base'] + (risk_factors['ade_score'] * self.risk_multipliers['ade_max'])
            )
            
            int_risk = max(
                base_predictions[0, 1].item(),
                self.risk_multipliers['interaction_base'] + (risk_factors['interaction_score'] * self.risk_multipliers['interaction_max'])
            )
            
            # Calculate weighted overall risk
            overall_risk = (
                ade_risk * self.risk_weights['ade'] +
                int_risk * self.risk_weights['interaction']
            )
            
            # Clamp values and return
            return {
                'ade_risk': min(max(ade_risk, 0.05), 0.95),
                'interaction_risk': min(max(int_risk, 0.05), 0.95),
                'overall_risk': min(max(overall_risk, 0.05), 0.95)
            }
