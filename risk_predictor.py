...
class RiskPredictor:
    ...
    def get_med_specific_features(self, cursor, patient_id: str, med_id: int) -> Dict:
        ...
        # Get interaction count - simplified query without CTE
        cursor.execute("""
            SELECT
        ...
        """, (patient_id, med_id, med_id, med_id))
        
        interactions = cursor.fetchone()
        
        interaction_score = 0.0
        if interactions['interaction_count'] > 0:
            major_score = (interactions['major_count'] or 0) * 0.25  # Reduced multiplier
            moderate_score = (interactions['moderate_count'] or 0) * 0.15  # Reduced multiplier
            
            interaction_score = min(major_score + moderate_score, 0.5)  # Lower cap
            
            total_meds = interactions['interaction_count']
            if total_meds > 3:  # Scale up only for multiple interactions
                interaction_score *= (1 + min(total_meds - 3, 3) * 0.1)  # Max 30% increase
